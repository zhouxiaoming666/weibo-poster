...
def post_weibo(
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            compose_selectors = [
                'div[role="textbox"], [class*="editor"], [class*="compose"]',
                'textarea[placeholder*="微博"], input[placeholder*="有什么新鲜事"]',
            ...
            if images:
                ...
                upload_selectors = [
                    ...
                    'button:has-text("图片"), [class*="image-upload"]',
                    '[class*="pic"], [class*="Pic"]'
                ]
                ...
                if file_input and file_input.input_enabled():
                    ...
                    # 尝试点击触发
                    try:
                        upload_btn = page.locator('button:has-text("图片"), [class*="image-btn"]').first
            ...
            if retry_count < retry_times:
                ...
                return post_weibo(config, content, images, video_path, topics, visible, script_dir, retry_count + 1)
...
def main():
    """主函数"""
    parser = argparse.ArgumentParser(description='微博发布工具（优化版）')
